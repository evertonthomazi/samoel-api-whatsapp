...
import { getWebhookPayload } from './controllers/common';
...
const MAX_DELAY: number = Number(process.env.MAX_DELAY || 0);
...
export async function sendPresenceUpdate(
  ...
  if (message.audio) {
    await session.sendPresenceUpdate('recording', jid);
  } else {
...
export async function createSession(options: createSessionOptions) {
  ...
  const socket = makeWASocket({
    printQRInTerminal: process.env.PRINT_QR_TERMINAL === "true" ? true : false,
  ...
  socket.ev.on('messages.upsert', async (m) => {
    ...
    const payload = await getWebhookPayload(message, session, sessionId);
    await webhook(payload);
  });
...
export async function jidExists(
  ...
  try {
    if (type === 'number') {
      ...
      if (exists) {
        try {
          url = await session.profilePictureUrl(result.jid, 'image');
        } catch (e) {
          url = null;
        }
      }
...
export async function canSendMessageToJid(
  session: Session,
  jid: string,
  type: 'group' | 'number' = 'number'
) {
